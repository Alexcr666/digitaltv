...
class WAColors {
  ...
  static const green = Color.fromARGB(255, 42, 47, 100);
...
class _KanbanConv {
  ...
  final String botId;
  SalesStage stage;
  _KanbanConv({required this.conv, required this.botId, required this.stage});
}
...
class _KanbanViewState extends State<_KanbanView> {
  ...
  Future<void> _load() async {
    setState(() => _loading = true);
    final List<_KanbanConv> result = [];
    for (final bot in widget.bots) {
      final convs = await widget.service.getConversations(bot.id);
      for (final conv in convs) {
        final stage = _stageMap[conv.id] ?? SalesStage.inicial;
        result.add(_KanbanConv(conv: conv, botId: bot.id, stage: stage));
      }
    }
    if (mounted) setState(() { _all = result; _loading = false; });
  }

  List<_KanbanConv> get _filtered {
    return _all.where((k) {
  ...
  Map<SalesStage, List<_KanbanConv>> get _grouped {
    final map = <SalesStage, List<_KanbanConv>>{};
    for (final s in SalesStage.values) map[s] = [];
    for (final k in _filtered) map[k.stage]!.add(k);
    return map;
  }

  void _moveStage(_KanbanConv item, SalesStage newStage) {
    setState(() {
      item.stage = newStage;
...
class _KanbanColumn extends StatelessWidget {
  ...
  final List<_KanbanConv> cards;
  final List<SalesStage> allStages;
  final Function(_KanbanConv, SalesStage) onMove;
  final Function(String convId, String botId) onOpenTrace;
